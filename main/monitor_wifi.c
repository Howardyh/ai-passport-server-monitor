...
#define GOT_IP BIT0
#define LOST_IP BIT1
typedef enum { CMD_RECONNECT, CMD_PROVISION, CMD_CONFIG } command_type_t;
...
static bool s_nvs_ok, s_sntp_ok;
...
bool monitor_wifi_config(monitor_config_t *c, uint32_t *revision) {
    ...
    lock();
    bool ready = s_view.connected && !s_view.provisioning && monitor_config_valid(&s_config);
    *c = s_config; *revision = s_view.revision;
    unlock();
    return ready;
}
void monitor_wifi_reconnect(void) {
    const command_t cmd = {.type = CMD_RECONNECT};
    if (s_commands) (void)xQueueSend(s_commands, &cmd, 0);
}
void monitor_wifi_provision(void) {
    const command_t cmd = {.type = CMD_PROVISION};
    if (s_commands) (void)xQueueSend(s_commands, &cmd, 0);
}
...
static esp_err_t portal_page(httpd_req_t *req) {
    ...
    snprintf(page, sizeof(page),
        "<!doctype html><meta name=viewport content='width=device-width'><title>Passport setup</title>"
        ...
        "<h1>Server Monitor setup</h1><p>Saved credentials are never displayed. Setup expires after 5 minutes.</p>"
        "<form id=f><input name=ssid placeholder='2.4 GHz Wi-Fi SSID' maxlength=32 required>"
        "<input name=password type=password placeholder='Wi-Fi password' minlength=8 maxlength=63 required autocomplete=new-password>"
        "<input name=url type=url placeholder='https://status.example.com/api/v1/status' maxlength=255 required>"
        ...
        "<button>Save and connect</button></form><p id=result></p><script>"
...
static esp_err_t portal_config(httpd_req_t *req) {
    ...
    command_t cmd = {.type = CMD_CONFIG, .config.version = 1};
    bool ok = root && cJSON_IsObject(root) &&
        ...
        copy_json_string(root, "token", cmd.config.token, sizeof(cmd.config.token)) && monitor_config_valid(&cmd.config);
...
static esp_err_t portal_start(void) {
    ...
    snprintf(name, sizeof(name), "Passport-%04x", (unsigned)(esp_random() & 0xffff));
    ...
    hc.stack_size = 6144; hc.max_open_sockets = 2; hc.lru_purge_enable = true;
...
static bool connect_station(void) {
    wifi_config_t wc = {0};
    lock(); monitor_config_t cfg = s_config; unlock();
...
static void publish_config(const monitor_config_t *cfg) {
    lock(); s_config = *cfg; s_view.revision++;
...
static void wifi_task(void *arg) {
    ...
    if (s_nvs_ok) (void)monitor_config_load(&loaded);
    ...
    for (;;) {
        ...
        if (connected) {
            ...
            if (!was_connected) { message("Wi-Fi connected"); if (s_sntp_ok) esp_netif_sntp_start(); }
...
esp_err_t monitor_wifi_start(void) {
    ...
    s_nvs_ok = nvs_flash_init() == ESP_OK;
